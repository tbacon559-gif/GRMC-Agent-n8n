...
export default async function InventoryDetailPage({ params }: PageProps) {
  ...
  const rawItem = await inventoryRepository.findById(id).catch((error) => {
  ...
  const [matches, customers] = await Promise.all([
    matchRepository.listForInventoryItem(id),
    customerRepository.list({ take: 200 }),
  ]);
    ...
    <div className="flex flex-col gap-6">
      ...
      {item.status !== "sold" && (
        <Card>
          ...
          <CardBody>
            <RecordSaleForm itemId={item.id} customers={customers.map((c) => ({ id: c.id, name: c.name }))} />
          </CardBody>
        </Card>
      )}
        ...
        <CardBody className="flex flex-col gap-3">
          {matches.length === 0 && (
            <p className="text-sm text-slate-500 dark:text-slate-400">
              No matches found — no open customer interests overlap with this item yet.
            </p>
          )}
          {matches.map((match) => {
            ...
            return (
              <div key={match.id} className="rounded-lg border border-slate-100 p-3 dark:border-slate-800">
                <div className="flex items-center justify-between">
                  <Link href={`/customers/${match.customerId}`} className="font-medium text-slate-900 hover:underline dark:text-slate-50">
                    {match.customer.name}
                  </Link>
