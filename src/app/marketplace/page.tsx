...
import { NewMarketplaceItemForm } from "@/modules/marketplace/components/NewMarketplaceItemForm";
...
export default async function MarketplacePage({ searchParams }: PageProps) {
  ...
  const items = (
    await marketplaceItemRepository.list({
      take: 100,
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-50">Marketplace</h1>
      ...
      <form className="flex flex-wrap gap-2 text-sm" action="/marketplace">
        <input
          name="search"
          defaultValue={search}
        ...
        {(search || status) && (
          <Link href="/marketplace" className="self-center text-xs text-slate-500 hover:underline dark:text-slate-400">
            Clear
          </Link>
        )}
      </form>
              ...
              {items.map((item) => (
                <tr key={item.id} className="border-b border-slate-50 last:border-0 dark:border-slate-800/60">
                  <td className="px-5 py-3">
                    <Link href={`/marketplace/${item.id}`} className="font-medium text-slate-900 hover:underline dark:text-slate-50">
                      {item.title}
                    </Link>
                  </td>
