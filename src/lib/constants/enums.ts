 ...
 *
 * These are plain string columns in Prisma, not native `enum` types, because
 * the SQLite connector Prisma uses for local dev does not support enums at
 * all (only Postgres/MySQL/CockroachDB do). Modeling them as validated
 * string unions here means the exact same schema.prisma works unchanged
 * against Postgres later. See docs/decisions/0003-no-native-enums-on-sqlite.md.
 */

export const CUSTOMER_STATUSES = ["prospect", "buyer", "seller"] as const;
