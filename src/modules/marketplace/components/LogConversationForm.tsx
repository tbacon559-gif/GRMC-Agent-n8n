 ...
 * pipeline (POST /api/marketplace/conversations) — the manual equivalent of
 * what the n8n Messenger automation does automatically.
 */
export function LogConversationForm({ contactId }: { contactId: string }) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const res = await fetch("/api/marketplace/conversations", {
      ...
      body: JSON.stringify({ contactId, rawText: form.get("rawText"), source: "manual" }),
    });
    setSubmitting(false);
    if (!res.ok) {
